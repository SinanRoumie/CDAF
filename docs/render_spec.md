...
*Measured (m1-v4).* Follow-up done. On a 20-site sample per kind — clean two-node co-introductions in the 1AC (empty prior, faithful since the 1AC has none), rendered under R2 and LLM-judged distinct-vs-collapsed — **impacts collapse 3/20 (15%)** and **frameworks 6/20 (30%)**. So the collapse is real for non-advocacy kinds but far from universal: the 82.0% surface overstates firing by roughly 3–6×, and frameworks (more abstract and paraphrasable) collapse about twice as often as impacts (they merge into "one systemic-consequences framework" / "one net-welfare framework"; impacts fuse when adjacent, e.g. energy and water harm read as one). Estimate only — n=20 per kind (wide CIs), impact sites fell in W2_B4 and framework sites in W3_B4 (the kinds cluster by workspace), one judge pass per site. These rates are measured *with* the Cause-B prompt fix (rule 6) already active but non-advocacy stems still per-kind (only advocacy got the per-node stem, Cause A) — so they are the residual the prompt fix alone leaves: rule 6 reduces same-kind collapse but does not eliminate it while the stems stay identical. Driving frameworks/impacts down further would need a per-node stem for those kinds too, not attempted here.
